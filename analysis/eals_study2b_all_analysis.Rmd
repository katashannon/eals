---
title: "EALS Study 2B Analysis: Learning Strategies, Multifunction Box (turned off, impossible to solve)"
...
eals2b_n32_df <- read_csv(here("data","exp2","EALS_Study2B_Data_N32.csv"))

# Read the merged data file into R
#eals2b_n32_df <- read.csv("/Users/Kat/Library/Mobile Documents/com~apple~CloudDocs/EALS/Code/EALS final repo/data/exp2/EALS_Study2B_Data_N32.csv")
...
eals2b_n32_df$condition <- factor(eals2b_n32_df$condition, levels = c("Reliable", "Unreliable"))
```
...
subset_eals2b_n32_df <- eals2b_n32_df %>%
...
summary_first_help <- eals2b_n32_df %>%
...
summary_bid_count <- eals2b_n32_df %>%
  group_by(condition) %>%
  summarise(
    total_N = n(),  
...
## Do children in the unreliable condition seek help less often and more slowly than their peers in the reliable condition (N = 32)?

```{r}
# prepare the survival analysis 
Surv(subset_eals2b_n32_df$first_help_bid.duration, subset_eals2b_n32_df$first_help_bid.count) 
```

```{r}
surv_model_eals2b_n32 <- survfit(Surv(first_help_bid.duration, first_help_bid.count) ~ condition, data = subset_eals2b_n32_df)
summary(surv_model_eals2b_n32)
```
...
ggsurvplot(surv_model_eals2b_n32, conf.int = TRUE, pval = TRUE, risk.table = TRUE,
...
```{r}
cox_model_eals2b_n32 <- coxph(Surv(first_help_bid.duration, first_help_bid.count) ~ condition, data = subset_eals2b_n32_df)
summary(cox_model_eals2b_n32)
```
...
```{r}
cox_eals2b_n32_df <- subset_eals2b_n32_df %>%
...
coxph_mod <- coxph_bf(data = cox_eals2b_n32_df,
         null_value = 0,
...
The Bayes factor for the preregistered hypothesis of a negative condition effect was \( \text{BF}_{10} = 0.316 \).

Let's confirm the inverse BF for the null hypothesis of no condition effect.

```{r}
BF_neg_vs_0 <- coxph_mod@bf
BF_0_vs_neg <- 1 / BF_neg_vs_0
print(BF_0_vs_neg)
```

The Bayes factor for null hypothesis \( \text{BF}_{01} = 3.16 \), so we have met our stopping rule for data collection at N = 32. 

## Exploration Analysis

Note that we met our stopping rule for data collection as noted above with N = 32. However, for consistency with Experiment 2A, we will conduct non-preregistered analyses to look at exploration although we are under powered to detect an effect.
...
eals2b_n32_df$condition <- factor(eals2b_n32_df$condition, levels = c("Unreliable", "Reliable"))

# descriptive statistics for toy_exploration.duration.sum by condition
summary_toy_exploration <- eals2b_n32_df %>%
  group_by(condition) %>%
  summarise(
...
ggplot(eals2b_n32_df, aes(x = condition, y = toy_exploration.duration.sum, fill = condition)) +
...
reliable_data <- eals2b_n32_df[eals2b_n32_df$condition == "Reliable",]$"toy_exploration.duration.sum"
unreliable_data <- eals2b_n32_df[eals2b_n32_df$condition == "Unreliable",]$"toy_exploration.duration.sum"

# wilcoxon rank-sum test 
wilcox_test_eals2b_n32 <- wilcox.test(reliable_data, unreliable_data, exact = FALSE)

print("Variable: toy_exploration.duration.sum")
print(wilcox_test_eals2b_n32)
```
The results of the wilcoxon rank-sum test show NO difference by condition in total time exploring the 4-button novel toy, p = 0.3
